...
public class PeerManager implements Runnable {
    ...
    synchronized void initializeReceivedParts(BitSet bitset) {
        for (RemotePeerInfo peer : _peers) {
            peer._receivedParts = ((BitSet)bitset.clone());
            if(peer.hasFile()){
                peer._receivedParts.set(0,peer._receivedParts.size()-1, true);
            }
        }
    }
